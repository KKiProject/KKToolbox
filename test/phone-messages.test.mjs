...
import { isPhoneIdentityEntry, loadPhoneIdentitySources, parsePhoneAiBundle, parsePhoneAiResponse } from '../phone-messages.js';
...
test('phone AI response accepts every supported simulated message type', () => {
    ...
    {"messages":[
        ...
        {"sender":"艾莉","type":"redpacket","content":"庆功","amount":8.88},
...
test('phone prompt lets AI use named stickers but never asks it to read their images', () => {
    const prompt = buildPhoneUserContent({
        ...
        snapshot: {
            profile: { nickname: '夜酱' },
            conversation: { type: 'group', name: '庆功群', members: ['艾莉', '西娅'] },
